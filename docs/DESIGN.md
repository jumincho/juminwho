...
- 논문 개최지는 `place: { city, country, countryCode }`로 적습니다. 국기는 `Flag.tsx`에 SVG로 그리고, 그림이 없는
  나라는 지도 핀으로 대신합니다. 새 나라가 필요하면 그림을 추가합니다.
...
- 목록마다 아이콘 붙이기(섹션 배지, 사실 행, 논문 개최지 국기에만 씀)
