...
- **마스코트 몽글이.** 새싹이 달린 복숭아 모찌입니다. 원본은 `public/favicon.svg` 한 파일이고, 파비콘, 헤더, 사진
  옆, 시계, 푸터, 앱 아이콘, OG 이미지에 모두 같은 그림을 씁니다.
- **Jumin 시계.** 아래 "Jumin 시계" 절을 따릅니다. 모든 상태 앞에는 "maybe…"를 붙입니다.
...
- **Local | Jumin 스위치.** 패널 제목 오른쪽(좁은 화면에서는 제목 아래 전체 폭)의 알약 스위치입니다. 라디오 버튼
  ...
  저장됩니다. 고른 시계가 큰 시계, 날짜, "until" 시각, 하루 리본에 모두 쓰입니다.
- **큰 시계.** 24시간제 `HH:MM`과 작은 초. Jumin 쪽은 국기와 "Jeonju, Republic of Korea", 방문자 쪽은 지구본과
  "Your time zone"을 붙이고, 그 아래에 시간대 이름과 UTC 차이를 적습니다. 매초 갱신합니다.
- **상태 카드.** Jumin 시간(KST) 기준 일과표로 정합니다. 구간은 시작한 날에 속하므로 금요일 밤 구간은 토요일
  새벽까지 이어집니다. 평일/주말은 Jumin 쪽 달력 날짜로 표시합니다.

  | 요일 | 시간(KST) | 상태 |
  | --- | --- | --- |
  | 평일 | 09:00–21:00 | maybe… working |
  | 평일 | 21:00–02:00 | maybe… in superposition (측정 전까지 여러 상태가 겹친 시간) |
  ...
  평일 01:00–02:00처럼 두 구간이 겹치면 두 상태를 함께 보여 주고, 배지 두 개를 겹쳐 그립니다. 일과표에 빈 시간이
  생기면 양자 중첩으로 봅니다.
- **하루 리본.** 고른 시계의 0–24시 위에 Jumin의 상태를 색 띠로 칠하고, 지금 위치에 몽글이를 세웁니다. 겹친
  시간은 두 색 줄무늬입니다. 범례도 "maybe…"로 시작합니다.
- **두 시계 비교.** 패널 맨 아래에 다른 쪽 시계의 시각과 "Jumin is 16 hours ahead of you." 같은 시차를 적습니다.
  같은 시간대면 "You’re on the same clock as Jumin."만 씁니다.
- 인쇄할 때는 숨깁니다.
...
- 시계: 양자 중첩 배지의 원자 회전 16초, 리본 위 몽글이 둥실, 스위치 손잡이 미끄러짐
- 이름 전환: 560ms 스프링
- 호버 시 살짝 뜨기: 3px 이하
...
- `npm run snapshots`가 통과한다. 375px과 1280px, 라이트와 다크에서 다음 항목을 검사하고 스크린샷을
  ...
  - 모든 상태가 "maybe…"로 시작하는지, 정해 둔 시각(로스앤젤레스에서 본 KST 9개 시각)의 상태가 일과표와 맞는지
  - 모션 줄이기에서 멈추는지
